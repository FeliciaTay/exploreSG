...
ui <- fluidPage(
  tabsetPanel(
    ...
    tabPanel("Tourist attractions", fluid = TRUE,
               mainPanel(
                 plotOutput("tourist")
               )
    ),
    ...
    tabPanel("Information about this app", fluid = TRUE,
             mainPanel(
              htmlOutput("title"),
              htmlOutput("desc")
             )
...
server <- function(input, output) {
  ...
  output$weather <- renderUI({ 
    ...
    data.long.lat <- read.csv("weather.csv")
    ...
    top <- head(weather.area.raw.data %>% rowwise() %>% mutate(dist = findDist(long, lat, avg_long, avg_lat)) %>% arrange(dist), 1)
  ...
  output$nearby <-renderLeaflet(
    {
      ...
      if (input$choose == "Hotels") {
        ...
        hawker <- read.csv("hawker_data_final.csv")
        hawker <- head(hawker %>% rowwise() %>% mutate(dist = findDist(Longitude, Latitude, avg_long, avg_lat)) %>% arrange(dist), 5)
